...
from space_computation import CollisionType, MovementType, Simulation, SpaceObject
...
def register_routes(app, socketio):
    ...
    def launch_simulation():
        ...
        try:
            ...
            simulation_time = data.get(
                "simulation_time", Simulation.__init__.__defaults__[1]
            )
            G = data.get("G", Simulation.__init__.__defaults__[2])
            collision_type = data.get(
                "collision_type", Simulation.__init__.__defaults__[3]
            )
            acceleration_rate = data.get(
                "acceleration_rate", Simulation.__init__.__defaults__[4]
            )
            elasticity_coefficient = data.get(
                "elasticity_coefficient", Simulation.__init__.__defaults__[5]
            )
            simulation: Simulation = Simulation(
                space_objects=[
                    SpaceObject(
                        ...
                        movement_type=MovementType(int(obj["movement_type"])),
                    )
                    for obj in data["space_objects"]
                ],
                time_delta=time_delta,
                simulation_time=simulation_time,
                G=G,
                collision_type=CollisionType(int(collision_type)),
            ...
            pools_dict[data["user_id"]] = SimulationExecutionPool(
                simulation=simulation,
                thread=Thread(
                    target=simulate, args=(request.json["user_id"], socketio)
                ),
                stop_event=Event(),
            )
